...
Spherical Harmonics help us describe scalar functions on the surface of a sphere. Without going into the mathematical details, they can be thought of describing a view direction dependent function on the sphere. For the purpose of 3DGS this *function* is the scalar colour value.
...
The associated Legendre polynomials $$P_l^m(x)$$ and $$P_l^{-m}(x)$$ generalise the Legendre polynomials $$P_l(x)$$ and are solutions to the associated Legendre differential equation, where $$l$$ is a positive integer and $$m=0, 1 
\ldots l$$. Same degree polynomials are orthogonal w.r.t. a constant term and between different degrees they are orthogonal with a different constant. The first few associated Legendre polynomials are given by the following expressions given [here](https://en.wikipedia.org/wiki/Associated_Legendre_polynomials#The_first_few_associated_Legendre_functions). I will list down the relevant ones for the purpose of this post
...
<!-- \label{sec:SH_coeff} -->
Spherical harmonics are the solution of Laplace's equation in spherical coordinates. The conversion from cartesian to spherical coordinates ([Figure 1](#fig-spherical-coord)) for a unit vector is simply
